...
import { updateGoal } from "../../redux/features/goal.slice";

ReactModal.setAppElement("#react-modal-portal");

const EditGoalModal = ({ isModalOpen, toggleModal, currentGoal }) => {
  const dispatch = useDispatch();
  const { token } = useSelector((state) => state.auth);

  const formik = useFormik({
    initialValues: {
      content: currentGoal ? currentGoal?.content : "",
      priority: currentGoal ? currentGoal?.priority : "low",
    },
    ...
    onSubmit: function (values) {
      ...
      dispatch(updateGoal({ goalObj: updatedGoal, token: token }));
      toggleModal();
    },
  });
